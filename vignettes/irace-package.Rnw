...
Timed out executions occur when the maximum execution bound (\parameter{boundMax}) is reached and the 
...
are penalized by multiplying  \parameter{boundMax} by a constant $X$. Using the \parameter{boundPar} option is possible to define the 
...
the evaluation of bounded executions by the \parameter{boundMax} value. More details about the implementation of adaptive capping can be found in \citet{PerLopHooStu2017:lion}.   
