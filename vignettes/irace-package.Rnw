...
\item \textbf{MPI}: By enabling the option \parameter{mpi}, calls
 ...
 \item \textbf{SGE cluster}: This mode uses the commands \code{qsub} and
   ...
   \end{xwarningbox}
 See the examples in
    \IRACEHOME{/examples/sge-cluster/}.
...
the example file (\code{target-runner}) provided in \IRACEHOME{/templates}. The
following examples are based on a file with those characteristics.
...
\item If your \code{targetRunner} script works when running irace with \code{parallel=1} but it fails when using higher number of cores, this may be due to any number of reasons:
  ...
  \item The above is particularly problematic when using MPI, because some
    calls to \code{targetRunner} may run on different computers than the one
    running the master \irace process. See FAQ in \autoref{sec:mpi-error}.
