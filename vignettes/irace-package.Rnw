...
\end{description}
For more information on \irace options and their default values see Section~\ref{sec:irace options}.
...
\item Modify the \code{target-runner} script to run your algorithm, this script
  ...
  \item The line provided in template:
    ...
    \code{CAND_PARAMS} adds to the command line the parameters that \irace has
...
\irace see Appendix~\ref{sec:check list} for a check list to diagnose common
problems. For more information about the \parameter{targetRunner} please see
Section~\ref{sec:runner},
        \end{enumerate}
...
described in Section~\ref{sec:parameters file}. Optionally, when executing
\irace from the \aR console, the parameters can be specified directly as an \aR
object (see Section~\ref{sec:parameters object}). For defining your parameters
