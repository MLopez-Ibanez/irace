...
\begin{itemize}
\item[] Manuel López-Ibáñez, Jérémie Dubois-Lacoste, Thomas Stützle, and Mauro Birattari. The \irace package, Iterated Race for Automatic Algorithm Configuration. Technical Report TR/IRIDIA/2011-004, IRIDIA, Université libre de Bruxelles, Belgium, 2011.\newline\url{http://iridia.ulb.ac.be/IridiaTrSeries/IridiaTr2011-004.pdf}
...
\item For GNU/Linux and OS X, the command-line executables \code{irace} and
  \code{parallel-irace} require GNU Bash. Individual examples may require
  additional software.
\end{itemize}
...
Follow the instructions in \code{forbidden.txt} and set \parameter{forbiddenFile}\code{="forbidden.txt"} in \code{scenario.txt}. More information about
...
need a value different than the default.
...
\item \textbf{MPI}: By enabling the option \parameter{mpi}, calls
 ...
 See the examples in
  ...
  \item \parameter{targetRunnerParallel}: This option allows users to fully
   ...
   @
   %
   where \code{experiments} is a list that contains elements with 
   information about configurations and instances to be executed (see
...
\MANUEL{LESLIE: CITE PAPER OF HOLGER} The columns \code{rho}, \code{KenW}, and
\code{Qvar} give the values of Spearman's rank correlation coefficient
rho, Kendall's concordance coefficient W, and a variance measure,
respectively, of the configurations across the instances executed so far in
this race.  Use \code{rho}, \code{KenW} and \code{Qvar} to analyze how
consistent is the performance of the configurations across the instances.  Note
that these values are only valid for the instances that were already executed
in the iteration.%
...
\code{Qvar} indicate that the performance is consistent and, therefore, the
scenario is homogeneous. For heterogeneous scenarios, we provide advice in
\autoref{sec:het}.
...
yet when the instance was used (only for the non elitist \irace),\MANUEL{I don't understand why it is ``only''} or 2) it was
...
<<parcord, fig.pos="H", fig.align="center", out.width='0.75\\textwidth', fig.cap="Parallel coordinate plots of the parameters of the configurations in the last two iterations of a run of \\irace.", prompt=TRUE, eval=TRUE, comment="">>=
...
hypervolume) may be used to evaluate the quality of the set.\footnote{An implementation is publicly available at \url{http://lopez-ibanez.eu/hypervolume}}
...
discarding configurations. The option \parameter{elitistInstances} in elitist
\irace (option \parameter{elitist}) can be used to increase the number of new
instances executed in each iteration, \eg \code{--elitist-instances 5} (default
...
budget. Different statistical tests uses different criteria to compare the
...
\defparameter{elitist}{elitist}{1} %
  ...
  instances. This number is defined by the option \parameter{elitistInstances}.
...
\url{http://cran.freestatistics.org/bin/macosx/}} %
...
You can install \aR from a CRAN mirror.\footnote{Belgian CRAN mirror:
  \url{http://cran.freestatistics.org/bin/windows/}} %
...
\item Make sure that your \code{targetRunner script} is at the specified location. If you see an error as:\\
...
(variable \code{EXE} for the templates example). A typical output for such an error is:\\
...
For testing your script, you can copy the command line shown in the error and
execute \code{target-runner} directly on the execution directory (\parameter{execDir}). In this case, the command line is:\\
...
algorithm correctly. To further investigate this issue, comment out the line that eliminates the
temporary files that saves the output of your algorithm. Similar to this one\\
