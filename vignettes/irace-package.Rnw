...
\end{description}
For more information on \irace options and their default values see \autoref{sec:irace options}.

\item Modify the \code{target-runner} script to run your algorithm, this script
  must execute your algorithm with the parameters and instances specified by
  \irace and return \textbf{only} one number corresponding to the evaluation of
  the execution.  The template we use in this guide is in \proglang{GNU Bash}
  ...
  \item The line provided in template:
    ...
    given for the execution.  You are free to set the command line execution as
   ...
   \MANUEL{Isn't this already explained a few lines above in page 9?}
    You can test the target runner from the \aR console by checking the scenario:
...
\MANUEL{This is a duplicate with what was mentioned at the start of the section. I actually would keep this version. It looks nicer.}
...
\renewcommand{\arraystretch}{1.2}
  \begin{tabularx}{0.98\linewidth}{@{}rX}
    \code{id.configuration}  & a numeric identifier that uniquely identifies a configuration;\\
    \code{id.instance} & a numeric identifier that uniquely identifies a pair \code{<instance, seed>};\\
...
targetRunner 1 113 734718556 /home/user/instances/tsp/2000-533.tsp \
...
parameter table. The program \parameter{targetRunner} must print (only) a real
number, which corresponds to the cost measure of the candidate configuration
for the given instance. The working directory of \parameter{targetRunner} is
...
\code{examples/hypervolume}. See also \autoref{sec:multi objective} for
...
default training instance directory is \path{./Instances}, you
...
in \autoref{sec:target parameters}.
...
\begin{xwarningbox}
Note that if initial configuration are provided (\autoref{sec:initial}), they must also comply with the
...
\item \textbf{MPI}: By enabling the option \parameter{mpi}, calls
 ...
 \item \textbf{SGE cluster}: This mode uses the commands \code{qsub} and
  ...
  \item \parameter{targetRunnerParallel}: This option allows users to fully
   ...
   \autoref{sec:runner} for a description), \parameter{targetRunner} is the
...
To set the testing instances follow the same guidelines as for the training instances (see \autoref{sec:training}).
...
\aR object. The structure of the object is described in \autoref{sec:output r}
...
not only contains the information of the tuning progress (See \autoref{sec:output r})
...
and execute \irace as described in \autoref{sec:execution}. For example from the command-line use:
...
effect the change will cause in the tuning. For questions contact us in the \irace group (\autoref{sec:contact}).
\end{xwarningbox}
...
corresponds to the index of the list found in \code{scenario$instancesList}, see \autoref{sec:output r}
