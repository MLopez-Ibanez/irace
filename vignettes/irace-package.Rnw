...
\item\hypertarget{opt:#2}{}\code{#2} ~~ flag: %
\ifthenelse{\equal{#1}{}}{%
\code{#1} or }{}%
\code{#3} ~~ default:~\textit{#4} \\
}
...
\irace receives as input a \code{parameter space definition} corresponding to the 
parameters of the target algorithm that will be tuned, a set of \code{instances} 
for which the parameters must be tuned for and a set of \code{options} for \irace.
...
\irace moves through the parameter search space in search of good performing 
algorithm configurations, for doing so \irace executes the target algorithm on 
different instances and with different parameter configurations. To execute 
the target algorithm with a specific parameter configuration ($\theta$) and instance ($i$) 
a \code{targetRunner} must be provided. The \code{targetRunner} acts as an
interface between the execution of the target algorithm and \irace, it receives
the instance and configuration as argument  and must return the evaluation of
the execution of the target algorithm.

The following user guide contains a guidelines to use \irace and define the needed 
components to execute \irace to automatically configure an optimization algorithm.
...
\begin{itemize}
 \item \aR (version $\geq~2.14.0$) is required for running irace, but you don't need to know the \aR language 
 to use it.  \aR is freely available 
 and you can download it from the \aR project website:
 \begin{center}
 \url{http://www.r-project.org}
 \end{center}
  
 See Section~\ref{sec:installation} for a quick installation guide of \aR.

 \item If you wish to use the command-line scripts \code{irace} and
   \code{parallel-irace}, they require GNU Bash.
\end{itemize}
...
\begin{enumerate}
\item  \textbf{Install automatically within \aR}: \\
...
If your run OS X you can use the \code{Packages and data -> Package installer} menu 
in the \aR console and on Windows you can use the \code{Packages} menu, follow the given 
instructions for installing the \irace package.

\item \textbf{Manual download and installation}: \\
Go to the \irace package CRAN website:
  \begin{center}
      \url{http://cran.r-project.org/package=irace}
   \end{center}

Download one of the three versions available depending of your operating
system: 
\begin{itemize}
...
The last command must print out the path of the library where \irace is installed. 
In the following of this document the variable \code{$IRACE_HOME} is used
in replacement of this path. When executing any provided command that includes 
the  \code{$IRACE_HOME} variable do not forget replace this variable with the 
installation path of \irace. If you are using GNU/Linux or OSx you can create the 
\code{$IRACE_HOME}  variable and add the path to your \code{.bash_profile}, 
\code{.bashrc} or \code{.profile} by executing the commands:
...
launch \irace as follows:

<<linux_irace_help,engine='bash',eval=FALSE>>=
irace --help
@

Unfortunately, this is not possible in Windows given that the command-line 
wrapper is not supported. To launch \irace, you need to open the \aR 
console and execute:
...
If the previous installation instructions fail to install the package because 
of insufficient permissions, you need to force a local installation. If you 
run GNU/Linux or OSx, execute the following commands to install the 
...
If you use Windows, you can install the package in a local directory by
executing the following lines in the \aR console:
...
Note that in both cases the directory where \irace will be installed \textbf{must} 
exist before attempting the installation. Since the \irace package is installed
in a local library, you \textbf{must} provide to \aR the path to this library 
when loading the package.

Once installed, load the package and test the correct installation by opening 
an \aR console and executing:

<<R_irace_test2, prompt=TRUE, eval=FALSE>>=
# Replace <installation path>' with the path used for 
# installation in the previous step.
...
The last line must show the installation path of \irace. In the following of this 
document the variable \code{$IRACE_HOME} is used in replacement of this 
path. When executing any provided command that includes 
the  \code{$IRACE_HOME} variable do not forget replace this variable with the 
installation path of \irace. If you are running GNU/Linux or OSx you can create 
the \code{$IRACE_HOME}  variable and add permanently the local library path 
to your \code{.bash_profile}, \code{.bashrc} or \code{.profile} by executing the 
following commands:

<<linux_irace_path2,engine='bash',eval=FALSE>>=
# Replace <installation path> with the path used for 
# installation in the previous step.
export IRACE_HOME=<installation path> 
export PATH=${IRACE_HOME}/bin/:$PATH
...
\section{Executing irace}\label{sec:execution}
...
want to adjust are:
\begin{itemize}
\item \code{execDir} (\code{--exec-dir}): set a directory in which \irace will execute the
target algorithm, the default value is the current directory.
\item \code{logFile} (\code{--log-file}): set the name of the results \aR data file that produces 
\irace.
\item \code{maxExperiments} (\code{--max-experiments}): set the maximum number of executions 
...
@
                ...
                about the \code{targetRunner} please see Section~\ref{sec:runner},
        \end{enumerate}
      
      \item \textit{Optional}: Modify the \code{target-evaluator} file. You can follow the guidelines provided for
            defining the \code{targetEvaluator} in Section~\ref{sec:evaluator}. 
...
a GNU/Linux system but making the necessary changes in the commands and \code{targetRunner},
...
\code{trainInstancesDir} and \code{trainInstancesFile}. The 
default training instance directory is \textit{./Instances}, you 
can modify this directory using the \code{trainInstancesDir} option 
(\eg~ \code{--train-instances-dir ~/my-instances/}). A file containing 
a list of instances can be specified using the \code{trainInstancesFile} 
...
line corresponds to the instance filename. If \code{trainInstancesDir} is 
...
If \code{./my-instances/} is provided as \code{trainInstancesDir},
\irace will assume instances are located in this directory, for example the first 
instance would be passed to \code{targetRunner} as \code{./my-instances/100/100-1_100-2.tsp}.
Disable the use of a directory by setting it as empty. Extra parameters are always passed 
to \code{targetRunner} together with the candidate configuration parameters values. 
...
The \code{deterministic} option activates the deterministic algorithm mode. Each instance will be
...
\begin{importantnote}
If \code{deterministic} is active and the number of training instances provided to \irace is less than
\code{firstTest} (default: 5), no statistical test will be performed on the race. 
\end{importantnote}

When \code{deterministic} is disabled \irace assigns each instance a seed once all the 
\code{<instance,seed>} pairs are used new pairs are generated with different seeds. 

The option \code{sampleInstances} enables the sampling of the instances, disable this
...
The scenario option \code{configurationsFile} allows to specify a set 
...
The scenario option \code{forbiddenFile} allows to specify a file containing 
...
\end{center}
The parameters are called by the parameter name set in the \code{parameterFile} described
...
\item \textbf{MPI}: By enabling the option \parameter{mpi}, calls
 ...
 \item \textbf{SGE cluster}: This mode uses the commands \code{qsub} and
   ...
   launch as many calls of \code{targetRunner} as possible and use \code{qstat}
   to wait for cluster jobs. The user \underline{must} call \code{qsub} from
   \code{targetRunner} with the appropriate settings for their cluster,
   otherwise \code{targetRunner} will not submit jobs to the cluster. In this
   ...
   \code{targetEvaluator} script is required to parse the results of
   \code{targetRunner} and return them to \irace. See the examples in
  ...
  \item \parameter{targetRunnerParallel}: This option allows users to fully
   ...
   Section~\ref{sec:runner} for a description), \code{targetRunner} is the
   \code{targetRunner} script or function and \code{scenario} is the scenario
   list. The \code{targetRunnerParallel} function must execute the
   \code{targetRunner} using the given experiments and scenario provided, and
   return a list of the same length as \code{experiments} containing the output
   of each call to \code{targetRunner}.
...
The options \code{testNbElites} and \code{testIterationElites} allow to set 
a testing of the best configurations found after the \irace execution has been completed.
The test instance set can be specified by the options \code{testInstancesDir} and \code{testInstancesFile} 
...
\begin{itemize}
\item \textbf{Final elite configurations testing}: \code{testNbElites} 
indicates the number of elite configurations to be tested, if 
the number of elite configurations is less than the value provided for \code{testNbElites}, 
then the number of elite configurations is used as \code{testNbElites}.

\item \textbf{Iteration elite configurations testing}: \code{testIterationElites} 
...
This line will load the \irace results found in the generated \code{logFile} 
...
To recover an \irace execution use the \code{recoveryFile} option and specify the \aR data log
...
You can specify the \code{recoveryFile} from the command-line or from the scenario file, 
...
the \code{logFile} option. 

\begin{importantnote}
You must define a different location for the \code{logFile} and the \code{recoveryFile}. 
...
\item \code{execDir}
\item \code{logFile}
\item \code{debugLevel}
\item \code{parallel}
\item \code{loadBalancing}
\item \code{mpi}
\item \code{sgeCluster}
\end{itemize}
...
This example can also can be applied to the target execution files \code{targetRunner} and \code{targetEvaluator}.
\begin{importantnote}
...
iteration an \aR data file is produced (\code{logFile} option), this file contains 
...
\item \code{|x|} : No statistical test was performed for this instance. To adjust in which 
      instances of a race statistical tests are performed see \irace options \code{firstTest} 
      and \code{eachTest} in Section~\ref{sec:irace options}.
...
\item \code{|!|}: This indicator exists only for the elitist version of \irace, indicates that
      ...
      option \code{elitist} in Section~\ref{sec:irace options} for more information.
\end{itemize}
...
The \aR data file created by \irace (\code{logFile}) contains an object called 
...
when finalizing the execution of \code{targetRunner} (see Section~\ref{sec:runner}) and specify 
a \code{targetEvaluator} in which the reference points are obtained and the hypervolume is calculated. 
For more information about defininf a \code{targetEvaluator} see Section~\ref{sec:evaluator}
...
computation time the execution time must be returned as result by the \code{targetRunner}. 
...
to always sample the instances initially (option \code{sampleInstances}) unless you 
...
before discarding configurations. Use the option \code{elitistInstances} when the elitist
\irace version (option \code{elitist}) is used to increase the number of new instances 
executed in each iteration. When using the non elitist \irace version you can indirectly 
increase the number of instances by increase the \code{firstTest} option. 
...
Using the option \code{confidence} is possible to set the statistical significance of 
the test. Increase the value of \code{confidence} to have a more strict statistical test.
...
the \code{targetRunner}. For example the following parameters:
...
The parameter values must be processed in the \code{targetRunner}. For example if 
...
transformations. Keep in mind that the \code{targetRunner} can also process the parameters.
You can also split parameters and join them in the \code{targetRunner}, for example assume the 
...
This parameters could be part of one parameters that has a multiplier and an exponent that has 
to be passed to your target algorithm as \code{--strenght }$m*e^10$. \code{targetRunner} can
...
The \irace option \code{targetRunnerRetries} indicates the number of times a
\code{targetRunner} execution is repeated if it fails. Use this option if you 
...
the \code{forbiddenFile} option to specify the configurations that must be 
avoided. On the other hand, if you do not know which configurations cause the 
problems, we advise you to handle this in the \code{targetRunner} script, when the program
...
\begin{itemize}
\item \code{scenarioFile} ~~ flag: \code{-s}, \code{--scenario}  ~~ default: \textit{./scenario.txt} \\
...
\item \code{debugLevel} ~~ flag: \code{--debug-level} ~~ default: \textit{0} \\
...
\item \code{seed} ~~ flag: \code{--seed} ~~ default: \textit{NA} \\ 
...
\item \code{execDir} ~~ flag: \code{--exec-dir} ~~ default: \textit{./} \\
...
\item \code{logFile} ~~ flag: \code{-l}, \code{--log-file} ~~ default: \textit{./irace.Rdata} \\  
  File to save tuning results as an R dataset, the provided path must be either an absolute path 
  or a relative to \code{execDir}. See Section\~ref{sec:output r} for details on the format of the 
  R dataset.
...
\begin{itemize}
\item \code{elitist} ~~ flag: \code{--elitist} ~~ default: \textit{1} \\
  ...
  have been executed, this number of instances can be defined with the option \code{elitistInstances}.
  ...
  options \code{firstTest} and \code{eachTest}, the elitist rule forbids to discard elite configurations,
  ...
  \code{deterministic} option is active, for which the instances are repeated if all instances have been used.    
  
\item \code{elitistInstances} ~~ flag: \code{--elitist-instances} ~~ default:\textit{1} \\
  Number of new instances to add to execution list before ``previous instances'' in elitist \irace.
  \begin{importantnote}
  If \code{deterministic} is \code{TRUE} then the number of \code{elitistInstances} will be reduced or 
  set to \code{0} in case no more instances are available.
  \end{importantnote}
  
\item \code{elitistLimit} ~~ flag: \code{--elitist-limit} ~~ default: \textit{2} \\
  Limit for the elitist race, when the number statistical test performed without successful elimination 
  reaches \code{elitistLimit} the race will be inmediately stopped. This limit has effect after all 
...
\begin{itemize}
\item \code{sampleInstances} ~~ flag: \code{--sample-instances} ~~ default: \textit{1} \\
  Enable/disable the sampleing of the the training instances. If the option \code{sampleInstaces} is disabled,
  the instances are used in the order provided in the \code{trainInstancesFile} or in the order that
  are read from the \code{trainInstancesDir} when\code{trainInstancesFile} is not provided. For more
  information about training instances see Section~\ref{sec:training}.
  
\item \code{nbIterations} ~~ flag: \code{--iterations} ~~ default: \textit{0} \\
...
\item \code{nbExperimentsPerIteration} ~~ flag: \code{--experiments-per-iteration}  ~~ default: \textit{0} \\
  ...
  tuning budget (\code{maxExperiments}), $\Bused$ is the used budget and $\Niter$ is maximum between the planned number of 
   iterations (\code{nbIterations}) and the current iteration ($j$). We recommend to use the default value.
  \begin{equation}
    \Budgetj = \dfrac{(\Budget - \Bused)}{ (\Niter -\iter + 1)}
  \end{equation}
  
\item \code{nbConfigurations} ~~ flag: \code{--num-configurations} ~~ default: \textit{0}\\
...
\item \code{softRestart} ~~ flag: \code{--soft-restart} ~~ default: \textit{1} \\
  ...
  by the hamming distance, the option \code{softRestartThreshold} defines the similatiry of numerical parameters.

  
\item \code{softRestartThreshold} ~~ flag: \code{--soft-restart-threshold} ~~ default: \textit{NA} \\
  Soft restart threshold value for numerical parameters. If \code{NA}, it computed as $10^-digits$, where 
  \code{digits} corresponds to the \irace option explained in this section.
...
\item \code{forbiddenFile} ~~ flag: \code{--forbidden-file} \\
...
\item \code{targetRunner}  ~~ flag: \code{--target-runner} ~~ default: \textit{./target-runner} \\
...
\item \code{targetRunnerRetries} ~~ flag: \code{--target-runner-retries} ~~ default: \textit{0} \\
  Number of times to retry a call to \code{targetRunner} if the call failed.
  
\item \code{targetRunnerData} 

\item \code{targetRunnerParallel} ~~ default: \code{""} \\
 Optional \aR function to provide custom parallelization of \code{targetRunner}. See Section~\ref{sec:parallel} 
 for more information.
...
\item \code{deterministic} ~~ flag: \code{--deterministic} ~~ default: \textit{0} \\
  Enable/disable deterministic algorithm mode, if enabled \irace will not use an instance more 
  than once in each race. See Section~\ref{sec:training} for more information.
  \begin{importantnote}
  Note that if the number of instances provided is less than the value
  specified for the option \code{firstTest}, no statistical test will be performed. 
  \end{importantnote}

\item \code{parallel} ~~ flag: \code{--parallel} ~~ default: \textit{0} \\
  Number of calls of the \code{targetRunner} to execute in parallel. A value of 0 means disabled. For more information 
  on parallelization see Section~\ref{sec:parallel}. 
  
\item \code{loadBalancing} ~~ flag: \code{--load-balancing} ~~ default: \textit{1} \\
...
\defparameter{mpi}{--mpi}{0} 
  Enable/disable MPI. Use \pkg{Rmpi} to execute the \code{targetRunner} in parallel. When \code{mpi} is 
  enabled, the option \code{parallel} is the number of slaves. See Section~\ref{sec:parallel}.
  
\defparameter{sgeCluster}{--sge-cluster}{0} 
  Enable/disable SGE cluster mode. Use qstat to wait for cluster jobs to finish (\code{targetRunner} must
...
\begin{itemize}
\item \code{configurationsFile} ~~ flag: \code{--configurations-file} \\
  ...
  \begin{importantnote}
    The provided configurations must not violate the constraints described in \code{parameterFile}
    and \code{forbiddenFile}.
  \end{importantnote}
\end{itemize} 
...
\begin{itemize}
\item \code{trainInstancesDir} ~~ flag: \code{--train-instances-dir} ~~ default: \textit{./Instances} \\
...
\item \code{trainInstancesFile} ~~ flag: \code{--train-instances-file} \\
  File containing a list of instances and optionally additional parameters for them. See Section~\ref{sec:training}.
  \begin{importantnote}
  If \code{trainInstancesDir} is specified the path contained in \code{trainInstancesFile} must be relative to the directory. For having 
...
\begin{itemize}
\item \code{maxExperiments} ~~ flag: \code{--max-experiments} ~~ default: \textit{1000}\\
  The maximum number of runs (invocations of \code{targetRunner}) that will be performed. It determines the maximum budget 
  of experiments for the tuning.
\end{itemize}
  
\subsection{Statistical test}
\begin{itemize}
\item \code{testType} ~~ flag: \code{--test-type} ~~ default: \textit{F-test} \\
...
\item \code{firstTest} ~~ flag: \code{--first-test} ~~ default: \textit{5} \\
  Specifies how many instances are executed before the first elimination test. 
  \begin{importantnote}
   The value of \code{firstTest} must be a multiple of \code{eachTest}.
  \end{importantnote}
  
\item \code{eachTest} ~~ flag: \code{--each-test} ~~ default: \textit{1} \\
  Specifies how many instances are executed between elimination tests.
  
\item \code{confidence} ~~ flag: \code{--confidence} ~~ default: \textit{0.95}\\ 
...
\begin{itemize}     
\item \code{recoveryFile} ~~ flag: \code{--recovery-file}  
...
\subsection{Testing}
\begin{itemize}  
\item \code{testNbElites} ~~ flag: \code{--test-num-elites} ~~ default: \textit{0} \\
  Number of elite configurations returned by irace that will be tested. For more information
  about the testing see Section~\ref{sec:testing}.
  
\item \code{testIterationElites} ~~ flag: \code{--test-iteration-elites} ~~ default: \textit{0} \\
  Enable/disable testing the elite configurations found at each iteration.

\item \code{testInstancesDir} ~~ flag: \code{--test-instance-dir} \\
  Directory where testing instances are located, either absolute or relative to current directory.
  
\item \code{testInstancesFile} ~~ flag: \code{--test-instance-file} \\
...
\subsection{Is \irace minimizing or maximizing the output of my algorithm?}
        ...
        By default, \irace considers that the value returned by \code{targetRunner} (or by
        \code{targetEvaluator}, if used) should be \underline{minimized}. In case of a maximization
     ...
     \begin{enumerate}
       \item{ Edit the \code{targetRunner} script to call MATLAB in a non-interactive
            way. See the MATLAB documentation, or the following links\footnote{\url{http://stackoverflow.com/questions/1518072/suppress-start-message-of-matlab}\\ \url{http://stackoverflow.com/questions/4611195/how-to-call-matlab-from-command-line-and-print-to-stdout-before-exiting}}. You would need to pass the parameter received by \code{targetRunner} to your MATLAB script: \url{http://www.mathworks.nl/support/solutions/en/data/1-1BS5S/?solution=1-1BS5S}. There is a minimal example in:
            \begin{center}
       ...
       \item{ Call MATLAB code directly from \aR using the
            ...
            is a better option if you are experienced in \aR. Define \code{targetRunner} as
  ...
  \subsection{My program works perfectly on its own, but not when running under \irace. Is irace broken?}
     ...
     the program, not in \irace.  We recommend that in \code{targetRunner}, you use
...
For questions and suggestions please contact the devolpment team through the mailing list:
...
\url{http://cran.freestatistics.org/bin/windows/}}. Once \aR is installed, 
you can launch the \aR console and install the \irace package from it 
(see Section~\ref{sec:irace install}).


\section{TargetRunner script check list} \label{sec:check list}

When the \code{targetRunner} script is not running properly can be difficult to detect where 
...
When you have problems with the \code{targetRunner} you will see an error on the \irace output
that says that the execution the \code{targetRunner} was not successful. 
...
output. If you see an error as follwing this is your problem:\\
