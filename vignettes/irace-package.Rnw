...
\subsubsection{Parameter file format}\label{sec:parameters file}\label{sec:forbidden}
  ...
  \begin{tabularx}{0.98\linewidth}{@{}rX}
   ...
   and, thus, it does not need to be tuned.\\
    ...
    \code{isDependent}  & Logical vector that specifies which parameter defines
                          a dependent domain.\\
    \code{digits}  & The number of digits for real valued parameters\\
...
See the examples in
  ...
  \item \parameter{targetRunnerParallel}: This option allows users to fully
   ...
   \code{exec.target.runner}. The user may load and call other \aR packages, such as \pkg{batchtools} (\url{https://mllg.github.io/batchtools/}).
...
corresponds to the row in the data frame returned by \code{get_instance_seed_pairs()}. See \autoref{sec:output r}
...
defined by the data frame returned by \code{get_instance_seed_pairs()}. The instance and seed used
...
index <- get_instance_seed_pairs(iraceResults)[pair.id, "instance"]
# Obtain the instance names
iraceResults$scenario$instances[index]
# Get the seeds
get_instance_seed_pairs(iraceResults)[pair.id, "seed"]
@
...
data frame returned by \code{get_instance_seed_pairs()}. When \parameter{capping} is enabled a column \code{bound} is added to log the execution bound applied 
for each execution.
...
\subsection{Warning: \code{In read.table(filename,}  \code{header = TRUE,} \code{colClasses =} \code{"character",} \code{: incomplete final line found by} \ldots}
