...
\item[] Manuel López-Ibáñez, Jérémie Dubois-Lacoste, Leslie Pérez Cáceres,
  ...
  Automatic Algorithm Configuration. \emph{Operations Research Perspectives},
  2016.
...
\item \textbf{MPI}: By enabling the option \parameter{mpi}, calls
 ...
 See the examples in
  ...
  \item \parameter{targetRunnerParallel}: This option allows users to fully
   ...
   \parameter{targetRunner} script or function provided by the user and 
   \code{scenario} is the scenario list. The \parameter{targetRunnerParallel} 
   function must execute the given \parameter{targetRunner} for each experiment in the 
   \code{experiments} list and using the provided \code{scenario} list, for example:
   
   <<targetRunnerParallel2,prompt=FALSE, eval=FALSE>>=
     # Option 1
     output.experiments <- list()
     for (i in 1:length(experiments))
       output.experiments[[i]] <- targetRunner(experiments[[i]], scenario)
     # Option 2
     output.experiments <- lapply(experiments, targetRunner, scenario=scenario)  
   @
   %
   The \parameter{targetRunnerParallel} function must return a list of 
   the same length as \code{experiments} containing the output of each call to
   \parameter{targetRunner}. Each \parameter{targetRunner} execution returns a list 
   containing two elements \code{<cost, time>}. When the time is not returned, the time 
   element of the list should be set as \code{NA}. If you decide not to use the 
   provided \code{targetRunner}, keep in mind that the output of each execution must be in this format,
   the following is an example of an output of a \parameter{targetRunnerParallel} with 2 experiments, 
   in which the execution time is not reported:
