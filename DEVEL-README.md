...
- –as-cran messages of the type:
  ...
  Undefined global functions or variables: median p.adjust pchisq qt
  read.table recover rexp rnorm runif t.test wilcox.test
  ...
  And adding the result to NAMESPACE. Namespaces newly imported from
  also need to be listed in the Imports: field of DESCRIPTION.

## RELEASE Process:

TODO: See useful release steps here:
<https://github.com/tidyverse/ggplot2/issues/4965>
...
6.  make examples \# Takes a few hours. Inspect the examples in the
    vignette.
...
9.  `cp cran-comments-template.md cran-comments.md` and update
    `cran-comments.md`

10. make submit

11. 1.  IF the package requires further changes:

- Make the changes.

- Repeat the whole RELEASE process above without bumping the version
  number.

11. 2.  IF the package is released in CRAN:
...
There are significant bugfixes and only very minor interface changes so
