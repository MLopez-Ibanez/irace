...
%The contents of this document are as follows.%  First we briefly
% introduce the automatic configuration problem, racing and \IFRACE,
% which are the ideas underlying the \irace software
% (Section~\ref{sec:prelim}). Next, we describe the general outline of
% the \irace software in Section~\ref{sec:irace}. The full details of
% installing, configuring and using \irace are described in
...
% Let $c^{\theta}_i$ denote the cost of a single run of configuration
% $\theta$ on instance $i_k$.

% If $\Theta_0$ denotes the set of initial candidate configurations,
% then
...
\textbf{FIXME: Mention what the defaults mean, for example, when nbIterations is 0, what it is really?}
...
The path can be given either as an absolute one or a relative one. In the latter case, the path is relative to the working directory.
...
\defparameter{execDir}{exec-dir}{./TUNE}
...
absolute or relative to \parameter{execDir}.
...
\defparameter{expName}{exp-name}{"Experiment Name"}
Experiment name for output report.

\defparameter{expDescription}{exp-description}{"Experiment Description"}
Longer experiment description for output report.

\defparameter{maxNbExperiments}{max-experiments}{1000}
...
limit (use \parameter{maxNbExperiments}).


\defparameter{timeEstimate}{time-estimate}{1}
...
\defparameter{maxInstances}{max-instances}{1000}
...
Specifies after how many instances are seen before each test after the first one is performed.

\defparameter{minNbSurvival}{min-survival}{0}
The minimum number of candidates that should survive to continue one iteration. Do not use something else than
 the default (that is, the dynamic value) unless you know exactly
 what you are doing.

\defparameter{nbCandidates}{num-candidates}{0}
The number of candidates that should be sampled and evaluated at each iteration. Do not use something else than
 the default (that is, the dynamic value) unless you know exactly
 what you are doing.
...
Number of calls to \parameter{hookRun} to execute in parallel. Less than 2 means calls to \parameter{hookRun} are sequentially executed.

\defparameter{sgeCluster}{sge-cluster}{0}
Enable/disable SGE cluster mode. Use qstat to wait for
cluster jobs to finish (\parameter{hookRun} must invoke qsub).

\defparameter{mpi}{mpi}{0}
 Enable/disable MPI. Use MPI to execute \parameter{hookRun} in parallel
 (the value of parameter \parameter{parallel} defines the number of slaves).
