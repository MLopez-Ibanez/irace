...
\item The configuration of \irace itself, which is defined by a number
  ...
  Section~\ref{sec:irace} to the configuration options in \irace. The
...
\subsection[Tuning optim() from R]{Tuning \code{optim()} from \aR}\label{sec:optim}
...
\begin{CodeInput}
R> hook.run <- function(instance, candidate, extra.params = NULL,
+                       config = list())
+  {
+    D <- 3
...
\subsection[Tuning ACOTSP]{Tuning \ACOTSP}\label{sec:acotsp}
...
default as \code{irace.Rdata}, which may be read from \aR
