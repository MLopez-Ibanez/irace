...
ablation_cmdline <- function(argv = commandArgs(trailingOnly = TRUE))
{
  ...
  params <- list()
  for (param in colnames(.ablation.params.def)) {
    value <- parser$readCmdLineParameter(paramName = param, default = params[[param]])
  ...
  ablog <- ablation(iraceResults = params$iraceResults, src = params$src, target = params$target,
                    ab.params = sapply(strsplit(params$ab.params, ",")[[1]], trimws, USE.NAMES=FALSE),
                    type = params$type, n.instances = params$n.instances, seed = params$seed,
