...
#' 
#' @param iraceResults (`list()`|`character(1)`) `iraceResults` object created by \pkg{irace} and typically saved in the log file `irace.Rdata`. Alternatively, the path to the log file from which the `iraceResults` object will be loaded.
...
#' ablog <- ablation(iraceResults = iraceResults, ablationLogFile = NULL)
#' plotAblation(ablog)
...
#' ablog <- ablation(iraceResults = irace.logfile, src = 1, target = 10,
                     ...
                     ablationLogFile = "log-ablation.Rdata", ...)
{
  ...
  if (is.character(iraceResults)) {
    load(iraceResults)
    if (is.character(iraceResults)) {
      stop("The file '", iraceResults, "' does not contain the 'iraceResults' object.")
    }
  }
