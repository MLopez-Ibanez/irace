...
checkScenario <- function(scenario = defaultScenario())
{
  ...
  if (is.null.or.empty(scenario$iterationCallback))
    scenario$iterationCallback <- NULL
  else if (is.function.name(scenario$iterationCallback))
    scenario$iterationCallback <- get.function(scenario$iterationCallback)
  else
    irace_error("'iterationCallback' must be a function")
...
#'      \item{`iterationCallback`}{Optional R function called after each irace iteration with the iteration number, ordered elite configurations, and progress information. (Default: `""`)}
#'      \item{`postselection`}{Perform a postselection race after the execution of irace to consume all remaining budget (requires elitist racing). Value 0 disables the postselection race. (Default: `1`)}
...
#'      \item{`minExperiments`}{Minimum number of runs (invocations of `targetRunner`) that will be performed. It determines the minimum budget of experiments for the tuning. The actual budget depends on the number of parameters and `minNbSurvival`. (Default: `NA`)}
