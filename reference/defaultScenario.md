...
- General options:
  ...
  - `iterationCallback`:
    ...
    Optional R function called after each irace iteration with the
    iteration number, ordered elite configurations, and progress
    information. (Default: `""`)

  - `postselection`:

    Perform a postselection race after the execution of irace to consume
    all remaining budget (requires elitist racing). Value 0 disables the
    postselection race. (Default: `1`)
