...
using command-line options, maybe parsed from the command line used to
invoke R.
...
- argv:
  ...
  e.g., `c("--scenario", "scenario.txt", "-p", "parameters.txt")`. Using
  the default value (not providing the parameter) is the easiest way to
  call `irace_cmdline()`.
...
invoke R, finds the name of the scenario file, initializes the scenario
from the file (with the function
[`readScenario()`](https://mlopez-ibanez.github.io/irace/reference/readScenario.md))
and possibly from parameters passed in the command line. It finally
starts irace by calling
[`irace_main()`](https://mlopez-ibanez.github.io/irace/reference/irace_main.md).
...
List of command-line options:
    ...
    -q,--quiet               Reduce the output generated by irace to a minimum.
       ...
       --debug-level         Debug level of the output of `irace`. Set this to 0 to
                             silence all debug messages. Higher values (1, 2 or 3)
                             provide more verbose debug messages. Default: 0.
    ...
    -e,--elitist             Enable/disable elitist irace. Default: 1.
       ...
       --bound-par           Penalization multiplication constant (PARX) for timed
                             out executions (executions that reach `boundMax`
       ...
       --postselection       Perform a postselection race after the execution of
                             irace to consume all remaining budget (requires
                             elitist racing). Value 0 disables the postselection
                             race. Default: 1.
