           ...
           int *nobjs_p, int **cumsizes_p, int *nsets_p)
{
	...
	nsets++; /* new data set */
        ...
        DEBUG3 (fprintf (stderr, "%s: set %d, read %d rows\n", 
...
{
    int k;
    fprintf (outfile, point_printf_format, data[0]);
    for (k = 1; k < dim; k++)
        fprintf (outfile, "\t" point_printf_format, data[k]);
}
            ...
            const int *cumsizes, int nruns)
{
    ...
    for (set = 0; set < nruns; set++) {
        for (; size < cumsizes[set]; size++) {
            write_point (outfile, &data[ncols * size], ncols);
            fprintf (outfile, "\n");
        }
        fprintf (outfile, "\n");
    }
